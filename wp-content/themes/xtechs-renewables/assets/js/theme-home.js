(function () {
  ...
  onReady(function () {
    ...
    var runCalc = function () {
      ...
      if (stcEl) stcEl.textContent = isValidVic ? stcCount.toString() : '0';
      if (stcValueEl) stcValueEl.textContent = isValidVic ? fmtCurrency(stcValue) : '$0';
      if (breakPvEl) breakPvEl.textContent = isValidVic ? fmtCurrency(stcValue) : '$0';
      if (vicEl) vicEl.textContent = isValidVic ? fmtCurrency(solarVictoriaRebate) : '$0';
      if (totalEl) totalEl.textContent = isValidVic ? fmtCurrency(totalIncentive) : '$0';
      ...
      if (runBtn) runBtn.textContent = isValidVic ? 'Calculate Rebates' : 'Enter Valid Postcode';
    };
    ...
    if (systemInput) systemInput.addEventListener('input', runCalc);
    if (batteryInput) batteryInput.addEventListener('input', runCalc);
    if (postcodeInput) postcodeInput.addEventListener('input', runCalc);
